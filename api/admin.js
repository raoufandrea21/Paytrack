...
export default async function handler(req, res) {
  ...
  if (action === 'usage') {
    try {
      const days = [];
      for (let i = 0; i < 31; i++) days.push(new Date(Date.now() - i * 86400000));
      const counts = await kvCmd(['MGET', ...days.map(d => usageKey(d))]);
      const rows = days.map((d, i) => ({ day: d.toISOString().slice(0, 10), requests: +(counts[i] || 0) }));
      const month = rows.filter(r => r.day.slice(0, 7) === rows[0].day.slice(0, 7))
        .reduce((t, r) => t + r.requests, 0);
      return res.status(200).json({ thisMonthRequests: month, days: rows });
    } catch (e) { return res.status(500).json({ error: e.message }); }
  }
